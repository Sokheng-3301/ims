<?php
    ...
    if(isset($_POST['add_schedule'])){
        ...
        if($major == '' || $room == '' || $start_time == '' || $end_time == '' || empty($_POST['subject'])  || empty($_POST['instructor'])
            ...
            // check schedule 
            $checkScheduleQry = mysqli_query($conn, "SELECT * FROM schedule_study
                                                     WHERE class_id = '". $class_id ."'
                                                     AND year_semester_id = '". $semester ."'
            ...
            if(mysqli_num_rows($checkScheduleQry) > 0){
                // Schedule has founded.
                $_SESSION['ADD_DONE_ERROR'] = 'This schedule has inserted already.';
                header("Location:". SITEURL ."add-schedule.php?dep=". $dep_id ."&maj=". $major. "&class=". $class_id);
                exit(0);
            }else{
                // Schedule has not founded.

                // check teacher in this schedule \\\\\\\\\\
                $teacherCheckQry = mysqli_query($conn, "SELECT * FROM schedule_study
                                                     WHERE day = '". $day ."'
                                                     AND instructor_id = '". $instructor ."'
                                                     AND start_time = '". $start_time ."'
                                                     AND end_time = '". $end_time ."'
                                                     AND year_semester_id ='". $semester ."'");
                if(mysqli_num_rows($teacherCheckQry) > 0){

                    $_SESSION['ADD_DONE_ERROR'] = 'Instructor has schedule already.';
                    header("Location:". SITEURL ."add-schedule.php?dep=". $dep_id ."&maj=". $major. "&class=". $class_id);
                    exit(0);
    ...
    if(isset($_POST['edit_schedule'])){
        ...
        $done_url = str_replace('sch='. $schId.'&', '', $url);
            ...
            if(mysqli_num_rows($checkScheduleQry) > 0){
                ...
                exit(0);
                    ...
                    if($update_schedule == true){
                        ...
                        header("Location:". SITEURL. $done_url);
                        exit(0);
                    }else{
